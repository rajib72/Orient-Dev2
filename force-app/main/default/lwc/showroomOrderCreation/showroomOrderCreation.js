...
import getShowroomVisit from '@salesforce/apex/ShowroomOrderController.getShowroomVisit';
import getWishlistItems from '@salesforce/apex/ShowroomOrderController.getWishlistItems';
import createOrder from '@salesforce/apex/ShowroomOrderController.createOrder';
...
import searchCustomersByPhone from '@salesforce/apex/ShowroomOrderController.searchCustomersByPhone';



export default class ShowroomOrderCreation extends NavigationMixin(LightningElement) {
        ...
        if (result.data) {
            this.wishlistItems = result.data.map(item => {
                ...
                return {
                    ...
                    // FULL CLASS STRING (THIS IS THE KEY)
    ...
    handleBillingName(e) { this.billingCustomerName = e.target.value; }
    //handleBillingPhone(e) { this.billingCustomerPhone = e.target.value; }
    ...
    handleRelation(e) { this.relation = e.target.value; }
    ...
    goStep2() {
        ...
        // re-sync selection + collapse all expanded rows
        this.wishlistItems = this.wishlistItems.map(item => ({
            ...item,
            isSelected: this.selectedWishlistIds.includes(item.id),

            // collapse table rows when moving next page
            isExpanded: false,
            expandIcon: 'utility:chevronright'
        }));
    }
    ...
    goStep3() {
        ...
        // collapse all expanded rows when moving from Step 2 to Step 3
        this.wishlistItems = this.wishlistItems.map(item => ({
            ...item,
            isExpanded: false,
            expandIcon: 'utility:chevronright'
    ...
    handleCreateOrder() {
        ...
        if (this.useAlternateCustomer && !this.gender) {
            this.toast(
                'Error',
                'Gender is mandatory for primary customer.',
                'error'
            );
            return;
        }
        ...
        createOrder({
            ...
            .then(orderId => {
                ...
                //  refresh wishlist status (Salesforce Mobile fix)
                refreshApex(this.wishlistWireResult);


                // Navigate to created Order
